            ...
            if (resultSet.rowsAffected > 0) {
              ...
              db.transaction((tx) => {
                tx.executeSql(
                  "SELECT * FROM farm_equipment",
                  null,
                  (resultSet) => setEquipment(resultSet.rows._array),
...
{/*  const showEquipment = () => {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql('CREATE TABLE IF NOT EXISTS farm_equipment_testssS (id INTEGER PRIMARY KEY AUTOINCREMENT, item_name TEXT, item_count INTEGER, item_model TEXT, item_serial TEXT)')
    });
    console.log(db)

    db.transaction(tx => {
      tx.executeSql('SELECT * FROM farm_equipment_testssS', null,
        (txObj, resultSet) => setEquipment(resultSet.rows._array),
        (txObj, error) => console.log(error)
      );
    });
  ...
  const addName = () => {
    db.transaction((tx) => {
      tx.executeSql(
        "INSERT INTO farm_equipment_testssS (item_name, item_count) VALUES (?,?)",
        [itemName, itemCount],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
          let existingNames = [...equipment];
          existingNames.push({ id: resultSet.insertId, name: itemName, count:itemCount});
          setEquipment(existingNames);
          setItemName(undefined);
        }
      },
        (error) => console.log("Error", error)
  ...
  const deleteName = (id) => {
    db.transaction(tx => {
      tx.executeSql('DELETE FROM farm_equipment_testssS WHERE id = ?', [id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            let existingEquipment = [...equipment].filter(item => item.id !== id);
  ...
  const showNames = () => {
    return equipment.map((item, index) => {
      return (
        <View key={index} style={styles.row}>
          <Text>{item.name}{item.id}{item.count}</Text>
  ...
  return (
    <View style={styles.container}>
      <StatusBar
        ...
        backgroundColor={
          Platform.OS === "ios" ? colors.white : colors.lightGray1
        }
      />
      ...
      <View style={styles.contentContainer}>
        <Text style={styles.headerTitle}>
          Enter Equipment/Machinery details
        </Text>
        <View
          style={{
        ...
        >
          <View style={{ paddingHorizontal: 25 }}>
            <View
              style={{ ...styles.historyTitleContainer, backgroundColor: "" }}
            >
              <View>
            ...
            <TextInput  placeholder='name' onChangeText={setCurrentName} />
            <Input
