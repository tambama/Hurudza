...
public partial class CustomMapHandler : MapHandler
{
    ...
    private static MKAnnotationView GetViewForAnnotations(MKMapView mapView, IMKAnnotation annotation)
    {
        ...
        else if(annotation is MKPolygonView polygonView)
        {
            var annotationCenter = polygonView.Polygon.Coordinate;
            var label = new UILabel(new CGRect(annotationCenter.Longitude, annotationCenter.Latitude, 100, 20));
            label.Text = "Polygon Center";
            label.TextAlignment = UITextAlignment.Center;
            label.BackgroundColor = UIColor.Clear;

            annotationView = new MKAnnotationView(annotation, "polygonLabel")
            {
                Frame = new CGRect(annotationCenter.Longitude, annotationCenter.Latitude, 100, 20),
